...
class mainWin(QtGui.QWidget):
    ...
    def initIcon(self):
        self.icon.showAction = QtGui.QAction("&显示窗口", self, triggered = self.showMain)
        self.icon.quitAction = QtGui.QAction("&退出", self, triggered = sys.exit)
    ...
    def leaveEvent(self, event):
        ...
        if self.right >= self.wid:
            self.setGeometry(self.wid - 2, 30, 150, self.hei)
    ...
    def react(self):
        sender = self.sender()
        os.startfile(self.findBtn(sender))
